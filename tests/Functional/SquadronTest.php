...
class SquadronTest  extends \TestCase {
    ...
    {
        $squadron = new Squadron;
        $squadron->add(WarriorCreator::Generate());
    ...
    public function test_that_a_defeated_warrior_is_not_counted()
    {
        ...
        $this->assertTrue(count($squadron) == 1, "Count of Squadron should return 1");
        WarriorCreator::Generate(["strength" => 1000000])->attack($warrior); // large attack will defeat immediately
    ...
    {
        $squadron = new Squadron;
        $warrior1 = WarriorCreator::Generate();
        $squadron->add($warrior1);

        $warrior2 = WarriorCreator::Generate();
        $squadron->add($warrior2);

        $this->assertTrue($warrior1 === $squadron->getNext(), "getNextWarrior should return the first warrior I added");

        WarriorCreator::Generate(["strength" => 1000000])->attack($warrior1); // large attack will defeat warrior1
    ...
    {
        $squadron = new Squadron(WarriorCreator::GenerateCollection(5));
